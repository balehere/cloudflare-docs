...
> 
> Custom Domains can stack on top of each other. For example, if Worker A is attached to `app.example.com` and Worker B is attached to `api.example.com`, Worker A can call `fetch()` on `api.example.com` and invoke Worker B.
> 
...
> 4. Enter the domain `baleoffline.org` you want to configure for your Worker.
...
> ```toml
> routes = [
> { pattern = "baleoffline.org", custom_domain = true }
> ]
> ```
> 
> </WranglerConfig>
...
> ```
> 
> However, if `worker-b` was instead set up to run on the Custom Domain `baleoffline.org`, the fetch request would succeed.
> 
...
> 
> 1. A Custom Domain for `baleoffline.org` points to your `api-worker` Worker.
> 2. A route added to `baleoffline.org/auth` points to your `auth-worker` Worker.
> 3. A request to `baleoffline.org/auth` will trigger your `auth-worker` Worker.
...
> 
> To migrate the route `baleoffline.org/*`:
> 
> 1. Log in to the [Cloudflare dashboard](https://dash.cloudflare.com) and select your account.
> 2. Go to **DNS** and delete the CNAME record for `baleoffline.org`.
> 3. Go to **Account Home** > **Workers & Pages**.
> 4. In **Overview**, select your Worker > **Settings** > **Domains & Routes**.
> 5. Select **Add** > **Custom domain** and add `baleoffline.org`.
> 6. Delete the route `baleoffline.org/*` located in your Worker > **Settings** > **Domains & Routes**.
> 
> ### Migrate from Routes via Wrangler
> 
> To migrate the route `baleoffline.org/*` in your `wrangler.toml / wrangler.json` file:
> 
> 1. Log in to the [Cloudflare dashboard](https://dash.cloudflare.com) and select your account.
> 2. Go to **DNS** and delete the CNAME record for `baleoffline.org`.
